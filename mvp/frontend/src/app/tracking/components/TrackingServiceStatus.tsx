...
import {
  IconCurrencyEthereum as IconBlockchain,
  IconDatabase,
  IconServer,
  IconMessageCircle2,
  IconClockHour4,
...
// Расширенный тип для статуса сервиса
...
export default function TrackingServiceStatus() {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      try {
        const response = await trackingApi.ping();
        setStatus(response as ServiceStatusType);
        setError(null);
      } catch (err) {
        console.error("Error checking tracking service:", err);
        setError("Не удалось подключиться к сервису отслеживания");
        setStatus(null);
      } finally {
    ...
    // Проверяем статус сервиса каждые 5 минут
      ...
      <Card withBorder p="sm" radius="md" bg="gray.1">
        <Group>
          <Text size="sm" fw={500} c="gray.7">
            Проверка статуса сервиса отслеживания...
          </Text>
        ...
        <Group justify="space-between">
          <Group>
            ...
            <Text size="sm" fw={600} c="green.8">
              Сервис отслеживания: {status.service} v{status.version}
            </Text>
          </Group>
          <Tooltip
              ...
              <Group align="flex-start" mt={8} wrap="nowrap">
                ...
                <div>
                  <Group gap={5}>
                    ...
                    <Badge
                      size="xs"
                      color={status.blockchain.connected ? "green" : "red"}
                    >
                      {status.blockchain.connected ? "Подключен" : "Отключен"}
                    </Badge>
                  </Group>
                  {status.blockchain.connected && (
                    <>
                      <Text size="xs">
                        Сеть: {status.blockchain.network} (Chain ID:{" "}
                        {status.blockchain.chainId})
                      </Text>
                      <Tooltip label={status.blockchain.contractAddress}>
                        <Text size="xs" truncate>
                          Контракт:{" "}
                          {status.blockchain.contractAddress.substring(0, 10)}
                          ...
                        </Text>
                      </Tooltip>
                    </>
